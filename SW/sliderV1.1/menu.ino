...
void initialize_menu() {
  // steplessmotion
  mu_steplessmotion.add_item(&mi_steplessmotion_start);
  mu_steplessmotion.add_item(&mi_steplessmotion_speed);
  mu_steplessmotion.add_item(&mi_steplessmotion_exit);
  ...
  // Root
  ms.get_root_menu().add_menu(&mu_steplessmotion);
...
//-------------------------------------------------------------------
//
void on_steplessmotion_start(MenuItem* p_menu_item)    // dorobit shutter
{ short microstepping_mode = 32;
  ...
  unsigned long lm_speed = steplessmotion_speed;
  ...
  if (steplessmotion_speed < 10) {
    ...
    if (steplessmotion_speed >= 10 and steplessmotion_speed < 50) {
      microstepping_mode = 32;
    }
    if (steplessmotion_speed >= 50 and steplessmotion_speed < 100) {
      microstepping_mode = 16;
    }
    if (steplessmotion_speed >= 100 and steplessmotion_speed < 200) {
      microstepping_mode = 8;
    }
    if (steplessmotion_speed >= 200 and steplessmotion_speed < 400) {
      microstepping_mode = 4;
    }
    if (steplessmotion_speed >= 400 and steplessmotion_speed < 800) {
      microstepping_mode = 2;
    }
    if (steplessmotion_speed >= 800 and steplessmotion_speed < 1000) {
...
{
  steplessmotion_speed = editNumericValue(1, steplessmotion_speed, 3, UNIT_STEPLESSMOTION_SPEED);
}
...
void display_menu(Menu const& menu) {
  ...
  if (strcmp(name, mi_steplessmotion_speed.get_name()) == 0) {
    displayNumericValue(1, steplessmotion_speed, 3, UNIT_STEPLESSMOTION_SPEED);
  }
